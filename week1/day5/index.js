...
const addNewKeyword = (label, keyword) => {
  ...
  if (keywords.includes(keyword)) {
    ...
    console.error(
      'It seems you forgot to write the label or the keyword in a function addNewKeyword'
    );
    return;
  } else {
...
const cleanedKeyword = (keyword) => {
  let reg = /[^A-Za-z0-9]/g;
  let cleanedKeyword = keyword.toLowerCase().replace(reg, '');
...
const reloadArticles = () => {
  ...
  data.articles.forEach((article) => {
    if (article.tags.some((x) => currentKeywords.includes(x))) {
...
const showKeywordsList = (value) => {
  ...
  if (value.length >= 3) {
    ...
    currentKeywords.forEach((x) => {
      console.log('test keyword added');
    });
